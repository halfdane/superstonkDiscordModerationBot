...
FakePost = namedtuple("Post", "permalink author id")


def fake_post(permalink="some_permalink", author="some_author", the_id="some_id"):
    return FakePost(permalink, author, the_id)
...
class TestPostCountLimiter:
    ...
    async def test_already_handled(self):
        ...
        mock_post_repo = AsyncMock()
        mock_post_repo.contains.return_value = True
        ...
        # then
        mock_post_repo.fetch.assert_not_called()
        mock_post_repo.do_not_count_to_limit.assert_not_called()
        mock_reddit.comment.assert_not_called()
        mock_bot.report_channel.send.assert_not_called()
    ...
    async def test_over_limit(self):
        ...
        mock_post_repo = AsyncMock()
        mock_post_repo.contains.return_value = False
        mock_post_repo.fetch.return_value = range(8)
